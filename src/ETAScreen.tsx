...
const whatToSay : string[] = [
  "Good Morning","First Period","10 mins Break","Second Period","10 mins Break","Third Period","Lunch Break","Forth Period" ,"10 mins Break", "Fifth Period" , "Classes Are Over For Today"
];
...
function getTimeRemaining(whatToSayIndex : number ,endtime : Date ) : TimeRemainingState
{
...
function whereAmI ( list : Date[] ) : TimeRemainingState {
  ...
  if (index === list.length){
    ...
    return getTimeRemaining(index,list[index]); 
}
function TimeRemainingStringify ( x : TimeRemainingState ) : string{
  let output : string = `` ;
  let h : string = x.hours.toString();
  let m :string  =  (x.minutes<10)? `0`+x.minutes.toString() : x.minutes.toString();
  let s : string =  (x.seconds<10)? `0`+x.seconds.toString() : x.seconds.toString();
  if(x.hours !==0 || x.minutes !== 0 || x.seconds !== 0 ){
    ...
    if (x.whatToSayIndex + 1 < whatToSay.length ){
      if (x.hours !== 0){
        ...
        output += x.seconds + ((x.seconds!== 1)? `   Seconds` : `   Second`) ;
      }
...
export function TimeRemaining (): JSX.Element{
  const [state , setState]  = useState <TimeRemainingState> (whereAmI(HScheduleDates));
  ...
  return (  
    <div className = "App-header">
     <Typography variant="h2" id="TimeRemaining" align="center">{TimeRemainingStringify(state)}</Typography>
    </div>
  );
