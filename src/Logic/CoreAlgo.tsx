 ...
 * @param m The Class we are pivoting to  
 * @param vertix The parent vertix to any vertix generated by this pivot
 * @param nextNode This will be added to the call stack and will be executed after any vertix generated by this pivot call finds a solution
 */
function pivotTo(vertix : callNodeType){
	...
	if (vertix.pivotArgs === undefined){
		...
		const nextNode  = vertix.pivotArgs.nextNode;
		const noRoot : boolean = vertix.pivotArgs.noRoot || false;
		const [X,Y] = vertix.Pos;
		const augmentedParent = (noRoot)?null:vertix;
		...
		teachers.forEach((replacementTeacher : string)=>{
			if (replacementTeacher === vertix.week.allClasses[m].l[X][Y].currentTeacher) return;
			const s = util.situation(replacementTeacher,vertix.Pos,m,vertix.week);
			if (s.r === -1){
				if (nextNode !== undefined){
					q.enqueue({callTo:'re',parent:augmentedParent,teacher:replacementTeacher,Pos:vertix.Pos,m,week:vertix.week,cycleClosingParentName:replacementTeacher,Action:s.action,Pivots:[...vertix.Pivots , nextNode]})
				}
				else{
					q.enqueue({callTo:'re',parent:augmentedParent,teacher:replacementTeacher,Pos:vertix.Pos,m,week:vertix.week,cycleClosingParentName:replacementTeacher,Action:s.action,Pivots:vertix.Pivots})
				}
			}
...
function re(vertix : callNodeType) {
	...
	if (!conflicts(vertix)) {
		...
		if (oldTeacher === "") {
			...
			// const requireWork: [number, number][] = 
			edges.filter((Pos1) => {
				...
				const newNode : callNodeType = {teacher:oldTeacher,Pos: Pos1,m,Pivots:vertix.Pivots,week,parent:vertix,callTo:'nothing'}
				const troubleMaker : ()=>boolean = ()=>{
					if (Action === "cycle" && vertix.cycleClosingParentName===undefined){
						...
						return vertix.cycleClosingParentName === t1
					}
				}
				if (
					(
				...
				if (S1.r === m) {
					...
					q.enqueue({
						callTo : 're', teacher: oldTeacher , Pos: Pos1, m,
						week, Action,Pivots:vertix.Pivots, parent: vertix,
...
const delegate = (
	...
	switch (situationInt(S)) {
		...
		case 2: // t==='' & r!==-1 & a ==='shift'
			...
			q.enqueue({...rootVertix, pivotArgs:{next_m:S.r}, callTo:'pivotTo'})
			break;
		...
		case 6: // t!=='' & r!==-1 & a ==='shift'
			...
			q.enqueue({
				...
				pivotArgs:{
					next_m: S.r,
					nextNode: {...rootVertix, callTo:'re', Action: S.action},
					noRoot: true
				}
			});
			break;
